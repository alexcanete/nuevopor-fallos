...
public class Controlador {
    ...
    public static void executeQuery() throws SQLException{
        ...
        String tableName = Principal.list.getSelectedItem();
        model.Conexion.setQuery("SELECT * FROM " + tableName);

    
            //mandar query a la base de datos
        Statement miOrden = dbms.DataBase.getCon().createStatement();
        miOrden.executeQuery("SELECT * FROM " + tableName);
    ...
    public static void  sqlNombreTablas() {
        Query = "SELECT TABLE_NAME FROM USER_TABLES ORDER BY 1";
        try {
            DefaultTableModel modelo = new DefaultTableModel();
            ...
            while(resultado.next()) {
                for(int iContador = 1; iContador <= numCampos; iContador++) {
                    ...
                    view.Principal.list.add(mostrar);
                }
            }
